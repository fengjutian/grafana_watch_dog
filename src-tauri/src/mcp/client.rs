...
impl GrafanaMcpClient {
    ...
    pub fn list_tools(&mut self) -> Result<Vec<ToolSummary>, String> {
        ...
        let tools = result.get("tools").and_then(Value::as_array).ok_or("MCP 响应缺少 tools")?;
        Ok(tools.iter().filter_map(|tool| {
            Some(ToolSummary {
                name: tool.get("name")?.as_str()?.to_owned(),
                description: tool.get("description").and_then(Value::as_str).unwrap_or_default().to_owned(),
            })
        }).collect())
    }
    ...
    fn request(&mut self, method: &str, params: Value) -> Result<Value, String> {
        let id = self.next_id;
        self.next_id += 1;
        self.send(JsonRpcRequest { jsonrpc: "2.0", id: Some(id), method, params: Some(params) })?;
        loop {
            let mut line = String::new();
            let bytes = self.stdout.read_line(&mut line).map_err(|e| format!("读取 MCP 响应失败：{e}"))?;
            if bytes == 0 { return Err("mcp-grafana 在响应前退出".into()); }
            let response: JsonRpcResponse = match serde_json::from_str(&line) { Ok(value) => value, Err(_) => continue };
            if response.id != Some(id) { continue; }
            if let Some(error) = response.error { return Err(format!("MCP error {}: {}", error.code, error.message)); }
            return response.result.ok_or("MCP 响应缺少 result".into());
        }
    }

    fn notify(&mut self, method: &str, params: Option<Value>) -> Result<(), String> {
        self.send(JsonRpcRequest { jsonrpc: "2.0", id: None, method, params })
    }
