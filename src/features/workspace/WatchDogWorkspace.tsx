...
import { generateReport, installMcpGrafana, listMcpTools, listReports, loadSettings, saveSettings, testConnection } from "../../infrastructure/tauri/client";
...
function SettingsPage({ initial, section }: { initial: AppSettings; section: "mcp" | "settings" }) {
  ...
  const install = async () => {
    ...
    try {
      ...
      setForm(next); await saveSettings(next);
  ...
  {isMcp && tools.length > 0 && <div className="tool-list"><b>已发现工具</b>{tools.slice(0, 8).map(tool => <span key={tool.name}><code>{tool.name}</code><small>{tool.description}</small></span>)}</div>}<div className="form-actions">{isMcp && <Button leftSection={<IconDownload size={16} />} onClick={install} loading={installing}>安装 mcp-grafana</Button>}{isMcp && <Button variant="default" onClick={test} loading={testing}>测试连接</Button>}<Button variant={isMcp ? "light" : "filled"} onClick={save}>保存设置</Button><span className="form-message">{message}</span></div></div><aside className="card safety"><span>♢</span><h3>默认安全策略</h3><ul><li>mcp-grafana 使用 --disable-write</li><li>建议使用 Viewer 最小权限账号</li><li>Token 与 API Key 不写入 SQLite</li><li>AI 仅能分析并提供建议</li></ul></aside></div></>;
}
