...
import type { AlertEvent, AppSettings, ConnectionDiagnostic, GrafanaDatasource, GrafanaDiscovery, Issue, McpTool, MetricSeriesPoint, Report, ServiceHealth, Status } from "../../domain/report/types";
...
function MetricPanel({ category, services, points, hours }: { category: "cpu" | "memory" | "disk" | "database"; services: Report["services"]; points: MetricSeriesPoint[]; hours: number }) {
  ...
  const option = {
    ...
    tooltip: { trigger: "axis", backgroundColor: "rgba(24,27,31,.96)", borderWidth: 0, textStyle: { color: "#fff", fontSize: 11 }, valueFormatter: (value: number) => category === "database" ? (Number(value) === 1 ? "正常" : "异常") : `${Number(value).toFixed(2)}%` },
    ...
    xAxis: { type: "time", min: Date.now() - hours * 3_600_000, max: Date.now(), boundaryGap: false, axisLine: { lineStyle: { color: "#cfd4da" } }, axisLabel: { color: "#7b828c", fontSize: 8, hideOverlap: true, formatter: hours > 24 ? "{MM}-{dd}\n{HH}:{mm}" : "{HH}:{mm}" }, splitLine: { show: true, lineStyle: { color: "#eef0f3" } } },
    yAxis: { type: "value", min: 0, max: category === "database" ? 1 : 100, interval: category === "database" ? 1 : undefined, axisLabel: { formatter: category === "database" ? (value: number) => value === 1 ? "UP" : "DOWN" : "{value}%", color: "#7b828c", fontSize: 8 }, splitLine: { lineStyle: { color: "#e8ebef" } } },
...
function Dashboard({ report, onGenerate, generating, points, settings, refreshing, onRefresh }: { report: Report; onGenerate: () => void; generating: boolean; points: MetricSeriesPoint[]; settings: AppSettings; refreshing: boolean; onRefresh: (hours: number, collect?: boolean) => void }) {
  ...
  useEffect(() => { discoverGrafana(settings).then(result => setDatasources(result.datasources)).catch(() => setDatasources([])); }, [settings.grafanaUrl, settings.selectedDatasourceUids.join("|")]);
  ...
  const sourceInfo = datasources.find(source => source.uid === datasource);
  return <>
    ...
    <div className="dashboard-filters"><label><span>数据源（名称 / 类型 / UID）</span><select value={datasource} onChange={event => { setDatasource(event.target.value); setJob("all"); setInstance("all"); }}>{datasourceOptions.map(uid => { const source = datasources.find(item => item.uid === uid); return <option key={uid} value={uid}>{source ? `${source.name} / ${source.kind} / ${uid}` : uid === "legacy" ? "历史旧数据 / 无法识别 / legacy" : uid}</option>; })}</select></label><label><span>Job</span><select value={job} onChange={event => setJob(event.target.value)}><option value="all">全部</option>{jobs.map(value => <option key={value}>{value}</option>)}</select></label><label><span>实例</span><select value={instance} onChange={event => setInstance(event.target.value)}><option value="all">全部</option>{instances.map(value => <option key={value}>{value}</option>)}</select></label><label><span>时间范围</span><select value={hours} onChange={event => { const value = Number(event.target.value); setHours(value); onRefresh(value); }}><option value={1}>最近 1 小时</option><option value={6}>最近 6 小时</option><option value={12}>最近 12 小时</option><option value={24}>最近 24 小时</option><option value={168}>最近 7 天</option></select></label><button className="dashboard-refresh" disabled={refreshing} onClick={() => onRefresh(hours, true)}><IconRefresh size={14} className={refreshing ? "spin" : ""} />{refreshing ? "采集中" : "立即采集"}</button><div className="source-lock">● {sourceInfo ? `${sourceInfo.name} · ${sourceInfo.kind}` : "历史旧数据"}<b>{datasource}</b></div></div>
    <section className="overview-panels four"><MetricPanel category="cpu" services={filtered} points={points.filter(point => point.datasourceUid === datasource && (job === "all" || point.job === job) && (instance === "all" || point.instance === instance))} hours={hours} /><MetricPanel category="memory" services={filtered} points={points.filter(point => point.datasourceUid === datasource && (job === "all" || point.job === job) && (instance === "all" || point.instance === instance))} hours={hours} /><MetricPanel category="disk" services={filtered} points={points.filter(point => point.datasourceUid === datasource && (job === "all" || point.job === job) && (instance === "all" || point.instance === instance))} hours={hours} /><MetricPanel category="database" services={filtered} points={points.filter(point => point.datasourceUid === datasource && (job === "all" || point.job === job) && (instance === "all" || point.instance === instance))} hours={hours} /></section>
