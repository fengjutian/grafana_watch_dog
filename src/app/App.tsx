...
function Dashboard({ report, onGenerate, generating }: { report: Report; onGenerate: () => void; generating: boolean }) {
  return <>
    <div className="page-title"><div><p className="eyebrow">DAILY OVERVIEW · {report.date}</p><h1>早上好，系统值得你关注一下。</h1><p>过去 24 小时的核心运行状态已经整理完毕。</p></div><button className="primary" onClick={onGenerate} disabled={generating}><span>{generating ? "◌" : "✦"}</span>{generating ? " 正在分析…" : " 生成今日日报"}</button></div>
    <section className="hero-grid">
      ...
      <div className="card conclusion"><div className="conclusion-top"><span className="ai-mark">✦</span><div><span className="section-kicker">AI CONCLUSION</span><h2>今日结论</h2></div></div><blockquote>{report.summary}</blockquote><div className="stat-row"><div><b className="red-text">{report.stats.critical}</b><span>严重问题</span></div><div><b className="amber-text">{report.stats.warning}</b><span>需要关注</span></div><div><b className="green-text">{report.stats.healthy}</b><span>正常指标</span></div><div><b>{report.stats.alerts}</b><span>昨日告警</span></div></div></div>
    </section>
...
export default function App() {
  ...
  return <div className="app"><aside className="sidebar"><div className="brand"><span>✦</span><div><b>AI Ops</b><small>DAILY</small></div></div><nav>{nav.map(n => <button key={n.id} className={page === n.id ? "active" : ""} onClick={() => navigate(n.id)}><i>{icons[n.id]}</i>{n.label}</button>)}</nav><div className="sidebar-bottom"><div className="connection"><i /><span><b>演示数据</b><small>等待 Grafana 配置</small></span></div><button className="profile"><span>CF</span><div><b>Ops Admin</b><small>本地工作区</small></div><i>•••</i></button></div></aside><main><header><button className="mobile-brand">✦</button><span>{title}</span><div><span className="readonly">◉ 只读模式</span><button className="icon-btn">◌</button></div></header><div className="content">{report && page === "dashboard" && <Dashboard report={report} onGenerate={run} generating={generating} />}{page === "reports" && !selected && <Reports reports={reports} onSelect={setSelected} />}{page === "reports" && selected && <><button className="back" onClick={() => setSelected(null)}>← 返回日报历史</button><Dashboard report={selected} onGenerate={run} generating={generating} /></>}{report && page === "analysis" && <Analysis report={report} />}{page === "mcp" && <SettingsPage initial={settings} section="mcp" />}{page === "settings" && <SettingsPage initial={settings} section="settings" />}</div></main>{toast && <div className="toast">✓ {toast}</div>}</div>;
}
